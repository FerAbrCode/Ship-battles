...
export function drawShip(ctx, ship, camX, camY, selected, state) {
  ...
  // Only show name tag for capitals (BB, CV), not escorts (DD, TB)
  if (ship.kind.type === 'BB' || ship.kind.type === 'CV') {
    const tagOffset = 18 * shipScale + 12; // Below the ship
  ...
  if (ship.alive) {
    ...
    if (st) {
      ...
      ctx.fillText(st, 0, -18 * shipScale - 8);
      ctx.globalAlpha = 1;
    }
  }
  ...
  const hpY = -14 * shipScale - 22; // Above the ship
...
// Also draws capital ship (non-commander BB/CV) trajectories
function drawEscortTrajectories(ctx, camX, camY, state) {
  if (!state.spectatorMode) return; // Only show in spectator mode
  
  const drawTrajectory = (ship, color, lockColor, isCapital = false) => {
    if (!ship.alive) return;
    if (!ship.targetWaypoint) return;
    
    // Skip commander ships - they don't follow formation
    if (ship.isCommander) return;
    
    // For escorts: DD and TB
    // For capitals: BB and CV that are NOT commander
    const isEscort = ship.kind.type === 'DD' || ship.kind.type === 'TB';
    const isFollowingCapital = (ship.kind.type === 'BB' || ship.kind.type === 'CV') && !ship.isCommander;
    
    if (!isEscort && !isFollowingCapital) return;
    ...
    // Draw lock radius circle around target waypoint (same for escorts and capitals)
    ...
    // Draw direct trajectory line
    ctx.strokeStyle = color;
    ctx.lineWidth = isCapital ? 2.5 : 1.5;
    ctx.setLineDash([4, 4]);
    ctx.globalAlpha = 0.6;
    ctx.beginPath();
    ctx.moveTo(sx, sy);
    ...
    ctx.setLineDash([]);
    ctx.beginPath();
    ctx.arc(tx, ty, isCapital ? 6 : 4, 0, TAU);
    ctx.fill();
    
    // Only show lock emoji if LOCKED (not unlocked)
    if (ship.escortLocked) {
      ctx.fillStyle = 'white';
      ctx.font = isCapital ? '14px Arial' : '12px Arial';
      ctx.textAlign = 'center';
      ctx.globalAlpha = 0.95;
      ctx.fillText('🔒', sx, sy - ship.kind.radius - 8);
    }
  ...
  // Draw collision predictions for all ships
  const drawCollisionPrediction = (ship) => {
    if (!ship.alive) return;
    if (!ship.collisionPredicted) return;
    
    const pred = ship.collisionPredicted;
    const cx = pred.x - camX;
    const cy = pred.y - camY;
    const sx = ship.x - camX;
    const sy = ship.y - camY;
    
    ctx.save();
    
    // Draw warning line from ship to collision point
    ctx.strokeStyle = `rgba(255, ${Math.round(255 * (1 - pred.urgency))}, 0, ${0.5 + pred.urgency * 0.4})`;
    ctx.lineWidth = 2 + pred.urgency * 2;
    ctx.setLineDash([6, 4]);
    ctx.beginPath();
    ctx.moveTo(sx, sy);
    ctx.lineTo(cx, cy);
    ...
    // Draw collision point marker (pulsing based on urgency)
    const pulseSize = 12 + Math.sin(state.time * 8) * 4 * pred.urgency;
    ctx.fillStyle = `rgba(255, ${Math.round(100 * (1 - pred.urgency))}, 0, ${0.6 + pred.urgency * 0.3})`;
    ctx.setLineDash([]);
    ctx.beginPath();
    ctx.arc(cx, cy, pulseSize, 0, TAU);
    ctx.fill();
    ...
    // Show time to collision
    ctx.font = '10px Arial';
    ctx.fillStyle = 'white';
    ctx.globalAlpha = 0.8;
    ctx.fillText(`${pred.time.toFixed(1)}s`, cx, cy + 18);
  ...
  for (const p of state.player) {
    const isCapital = p.kind.type === 'BB' || p.kind.type === 'CV';
    if (isCapital && !p.isCommander) {
      // Non-commander capital - gold color
      drawTrajectory(p, 'rgba(255,215,0,0.8)', 'rgba(255,255,100,0.6)', true);
    } else {
      // Escort - cyan
      drawTrajectory(p, 'rgba(100,220,255,0.8)', 'rgba(100,255,100,0.6)', false);
    }
    // Draw collision prediction
    drawCollisionPrediction(p);
  }
  
  // Draw enemy trajectories
  for (const e of state.enemy) {
    const isCapital = e.kind.type === 'BB' || e.kind.type === 'CV';
    if (isCapital && !e.isCommander) {
      // Non-commander capital - orange color
      drawTrajectory(e, 'rgba(255,165,0,0.8)', 'rgba(255,200,100,0.6)', true);
    } else {
      // Escort - pink
      drawTrajectory(e, 'rgba(255,150,180,0.8)', 'rgba(255,200,100,0.6)', false);
    }
    // Draw collision prediction
    drawCollisionPrediction(e);
  }
}
